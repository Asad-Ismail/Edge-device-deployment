...
static {
    ...
    public void run() {
        ...
        Log.d("D2Go",  " Inference Time is : " + inferenceTime);
        ...
        if (map.containsKey("boxes")) {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                final Tensor maskTensor = map.get(String.valueOf(i)).toTensor();
                maskData =maskTensor.getDataAsFloatArray();
                // Add Indiviual Masks to List
                masks.add(maskData);
                count++;
            }
            Log.d("D2Go",  " maskData length is : " + masks.size());
            final ArrayList<Result> results = PrePostProcessor.outputsToPredictions(count, outputs, masks, mImgScaleX, mImgScaleY, mIvScaleX, mIvScaleY, mStartX, mStartY);
            runOnUiThread(() -> {
